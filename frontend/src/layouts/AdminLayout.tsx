...
const AdminLayout: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Sidebar */}
      <aside
        className={`${isMobile ? (showMobileSidebar ? 'translate-x-0' : '-translate-x-full') : ''} ${
          isCollapsed && !isMobile ? 'w-20' : 'w-72'
        } ${isMobile ? 'fixed inset-y-0 left-0 z-50' : 'relative'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-xl flex flex-col transition-all duration-300 ease-in-out overflow-visible`}
      ...
      >
        ...
        >
          <div className={`flex items-center ${isCollapsed && !isMobile ? 'justify-center' : ''}`}>
            ...
            {(!isCollapsed || isMobile) && (
              <div className="ml-3">
                <h1 className="text-lg sm:text-xl font-bold text-white">GreenMart</h1>
                <p className="text-xs text-green-100 hidden sm:block">Admin Dashboard</p>
              </div>
            )}
          </div>
          {!isMobile && (
        ...
        {/* Profile Section */}
        <div className="p-2 sm:p-4 border-t border-gray-200 relative">
          <button
            onClick={() => setShowProfile(!showProfile)}
            className={`w-full flex items-center ${isCollapsed && !isMobile ? 'justify-center' : 'gap-3'} p-2 sm:p-3 rounded-xl hover:bg-gray-50 transition-colors relative group`}
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-green-500 to-green-600 rounded-lg flex items-center justify-center text-white font-bold text-sm sm:text-base">
              A
            </div>
            {(!isCollapsed || isMobile) && (
              <>
      ...
      >
        {/* Top Header */}
        <header
          className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-8 py-3 sm:py-4 shadow-sm"
          style={{ backgroundColor: isDarkMode ? '#111827' : '#fff' }}
        >
          <div className="flex items-center justify-between">
            {/* Mobile menu button and page info */}
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <span className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 bg-red-500 rounded-full text-xs text-white flex items-center justify-center font-bold">
