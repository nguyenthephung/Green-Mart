...
const Checkout = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (validItems.length === 0) {
        alert(
          'Không có sản phẩm hợp lệ trong giỏ hàng. Có thể do dữ liệu sản phẩm bị lỗi. Vui lòng thêm lại sản phẩm vào giỏ hàng.'
        );
        return;
      }
        ...
        if (paymentMethod === 'momo' || paymentMethod === 'paypal') {
          ...
          try {
            ...
            if (paymentResponse.success && redirectUrl) {
              ...
              alert(
                `Không thể tạo link thanh toán ${paymentMethod.toUpperCase()}. Vui lòng thử lại hoặc chọn phương thức thanh toán khác.`
              );
            ...
            // ...existing code (đã xóa log)...
            alert(
              `Thanh toán ${paymentMethod.toUpperCase()} thất bại: ${paymentError instanceof Error ? paymentError.message : 'Lỗi không xác định'}. Vui lòng thử lại hoặc chọn phương thức thanh toán khác.`
            );
            setIsProcessingOrder(false);
            return;
          }
        ...
        if (paymentMethod === 'cod' || paymentMethod === 'bank_transfer') {
          try {
            ...
            if (paymentResponse.success) {
              ...
              navigate(`/order-success?orderId=${orderId}&orderNumber=${orderNumber}`, {
                replace: true,
              });
              return;
            } else {
              // ...existing code (đã xóa log)...
              alert(
                `Không thể tạo bản ghi thanh toán. Đơn hàng đã được tạo nhưng có thể cần liên hệ admin.`
              );
              setIsProcessingOrder(false);
              return;
            }
            ...
            // ...existing code (đã xóa log)...
            alert(
              `Đơn hàng đã được tạo nhưng có lỗi xử lý thanh toán. Vui lòng liên hệ admin với mã đơn hàng: ${orderNumber}`
            );
            setIsProcessingOrder(false);
            return;
          }
      ...
      if (error instanceof Error) {
        if (error.message.includes('Insufficient stock')) {
          ...
          alert(
            `Xin lỗi! Sản phẩm "${productName}" không đủ số lượng trong kho. Vui lòng giảm số lượng hoặc xóa khỏi giỏ hàng và thử lại.`
          );
  ...
  return (
    <div className="bg-gradient-app-main min-h-screen flex flex-col">
          ...
          <div className="text-center">
            ...
            <div className={`${isMobile ? 'text-base' : 'text-lg'} text-app-secondary break-words`}>
              {cartLoading ? (
                ...
                <>
                  {cart.length} sản phẩm • Tổng tiền:{' '}
                  <span className="font-semibold break-all">{subtotal.toLocaleString()}</span> ₫
                  {!isAuthenticated && (
      ...
      <main
        className={`max-w-7xl mx-auto ${isMobile ? 'px-2 pb-4' : 'px-4 pb-8'} ${isMobile ? 'grid grid-cols-1 gap-4' : 'grid grid-cols-1 lg:grid-cols-3 gap-8'} flex-grow`}
      >
          ...
          {canCheckout ? (
            ...
            <div className="bg-app-card rounded-2xl shadow-lg p-8 border border-orange-100">
              <div className="text-center">
                ...
                <p className="text-app-secondary mb-6">
                  Bạn cần thêm địa chỉ giao hàng để tiếp tục thanh toán.
                </p>
                <div className="flex gap-4 justify-center">
                  <button
                ...
                <div className="flex-1 p-3 bg-green-50 border border-green-200 rounded-xl">
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0 flex-1">
                      <span className="font-semibold text-green-700 block truncate">
                        {voucher.code}
                      </span>
            ...
            <div className="bg-app-card rounded-2xl shadow-lg p-6 border border-red-100">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm text-app-secondary">
                  ...
                  <span className="font-medium text-app-primary break-all">
                ...
                <div className="border-t border-app-border pt-3 flex justify-between font-semibold text-app-primary">
                  ...
                  <span className="break-all">
                    {(subtotal - voucherDiscount).toLocaleString()} ₫
                  </span>
                </div>
                <button
