...
const GuestFlashSalePage: React.FC = () => {
  const { activeFlashSales, upcomingFlashSales, fetchActiveFlashSales, fetchUpcomingFlashSales } = useFlashSaleStore();
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-yellow-50 via-orange-50 to-red-50 py-0">
      <div className="w-full px-0">
        <h1 className="text-4xl font-black text-center mb-8 text-orange-700 tracking-tight py-8 bg-gradient-to-r from-orange-100 to-yellow-100">FLASH SALE HOT NHẤT</h1>
          ...
          <section key={cat._id} className="w-full mb-16">
            ...
            {categoryBanners[cat.id] && (
              <div className="mb-6 flex justify-center w-full">
                <img src={categoryBanners[cat.id].imageUrl} alt={categoryBanners[cat.id].title} className="rounded-2xl shadow-2xl max-h-56 object-cover w-full" />
              </div>
            )}
            <h2 className="text-3xl font-bold text-orange-700 mb-6 text-center tracking-wide uppercase">{cat.name}</h2>
            ...
            {salesByCategory[cat.name]?.length ? (
              salesByCategory[cat.name].map((sale, saleIdx) => {
                ...
                if (!products.length || (time.hours === 0 && time.minutes === 0 && time.seconds === 0)) return null;
                return (
                  <div key={sale._id + '-' + saleIdx} className="relative w-full bg-gradient-to-r from-orange-100 to-yellow-100 rounded-2xl shadow-2xl p-8 flex flex-col items-center mb-8">
                    {/* Countdown cho khung giờ này */}
                    <div className="flex items-center gap-4 mb-6 justify-center">
                      <Flame className="w-8 h-8 text-yellow-500 animate-pulse" />
                      <span className="font-bold text-orange-700 text-lg">Khung giờ: {`${new Date(sale.startTime).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })} - ${new Date(sale.endTime).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}`}</span>
                      <span className="font-semibold text-orange-600">Kết thúc trong:</span>
                      <div className="flex gap-1">
                        <span className="bg-orange-400 text-white px-3 py-2 rounded text-lg font-mono min-w-[2rem] text-center shadow-lg">{String(time.hours).padStart(2, '0')}</span>
                        <span className="text-orange-700 font-bold text-lg">:</span>
                        <span className="bg-orange-400 text-white px-3 py-2 rounded text-lg font-mono min-w-[2rem] text-center shadow-lg">{String(time.minutes).padStart(2, '0')}</span>
                        <span className="text-orange-700 font-bold text-lg">:</span>
                        <span className="bg-orange-400 text-white px-3 py-2 rounded text-lg font-mono min-w-[2rem] text-center shadow-lg">{String(time.seconds).padStart(2, '0')}</span>
                      </div>
                    </div>
                    <div className="flex gap-6 overflow-x-auto scrollbar-thin scrollbar-thumb-orange-300 scrollbar-track-orange-100 w-full justify-center">
                      {products
                        .slice(currentIndexes[cat.name + '-' + sale._id] || 0, (currentIndexes[cat.name + '-' + sale._id] || 0) + productsPerView)
                        .map((product: any, idx: any) => {
                          ...
                          return (
                            <div key={sale._id + '-' + idx} onClick={handleClick} className="min-w-[240px] max-w-[240px] bg-white rounded-2xl p-6 cursor-pointer hover:shadow-2xl transform hover:scale-105 transition-all duration-200 border-2 border-orange-200 flex flex-col items-center">
                              <img src={product.product?.image || '/placeholder-image.jpg'} alt={product.product?.name || 'Product'} className="w-full h-40 object-cover rounded-xl mb-3 shadow-lg" />
                              <h3 className="font-bold text-orange-700 mb-2 text-base text-center max-w-[220px] overflow-hidden whitespace-normal break-words line-clamp-2" style={{display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{product.product?.name || 'Sản phẩm không tên'}</h3>
                              <div className="flex flex-col gap-1 mb-3 items-center">
                                <span className="text-red-600 font-bold text-xl">{product.flashSalePrice?.toLocaleString('vi-VN')}₫</span>
                              ...
                              <div className="mb-3 w-full">
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                  <div className="bg-gradient-to-r from-orange-400 to-red-500 h-2 rounded-full" style={{ width: `${product.quantity > 0 ? (product.sold / product.quantity) * 100 : 0}%` }} />
                                </div>
                              </div>
                            </div>
                          );
                    ...
                    {products.length > productsPerView && (
                      <>
                        <button
                          onClick={() => setCurrentIndexes(idx => ({ ...idx, [cat.name + '-' + sale._id]: Math.max((idx[cat.name + '-' + sale._id] || 0) - 1, 0) }))}
                        ...
                        <button
                          onClick={() => setCurrentIndexes(idx => ({ ...idx, [cat.name + '-' + sale._id]: Math.min((idx[cat.name + '-' + sale._id] || 0) + 1, products.length - productsPerView) }))}
                          disabled={(currentIndexes[cat.name + '-' + sale._id] || 0) >= products.length - productsPerView}
              ...
              <div className="text-center text-gray-400 italic">Chưa có flash sale cho danh mục này</div>
            )}
          </section>
        ))}
      </div>
