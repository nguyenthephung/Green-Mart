...
const OrdersPage = () => {
  ...
  const handleOrderCancelled = (orderId: string) => {
    setOrders(prev =>
      prev.map(order => (order.id === orderId ? { ...order, status: 'Đã hủy' } : order))
    );
  };
  ...
  const tabKeys = ['Tất cả', 'Chờ xác nhận', 'Chờ giao hàng', 'Đã hủy'];
  const tabCounts = tabKeys.reduce(
    (acc, key) => {
      ...
      >
        ...
        {!loading && !error && (
          <>
            {/* Stats Cards */}
            <div
              className={`grid ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-2 md:grid-cols-4 gap-4'} mb-8`}
            >
              {tabKeys.map(tab => (
                <div key={tab} className="bg-app-card rounded-2xl p-4 shadow-lg border-app-default">
                  <div className="text-center">
                    <div
                      className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-brand-green`}
                    >
                      {tabCounts[tab]}
                    </div>
                    <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-app-secondary mt-1`}>
                      {tab}
                    </div>
                  </div>
                </div>
                ...
                {filteredOrders.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <h3 className="text-xl font-semibold text-app-primary mb-2">
                      {activeTab === 'Tất cả'
                        ...
                        : `Không có đơn hàng ${activeTab.toLowerCase()}`}
                    </h3>
                  ...
                  <div className="space-y-6">
                    {filteredOrders.map(order => {
                      ...
                      const shippingStatus = tracking
                        ? `${tracking.status} tại ${tracking.address}`
                        : '';
                      return (
                        <OrderCard
                          key={order.id}
