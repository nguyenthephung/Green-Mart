...
const AdminVouchersPage: React.FC = () => {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await import('../../services/voucherService').then(s =>
        s.voucherService.update(editVoucher.id, submitForm)
      );
      await fetchVouchers();
  ...
  const [sortField, setSortField] = useState<'label' | 'discountValue' | 'expired' | 'isActive'>(
    'label'
  );
  ...
  const sortedVouchers = [...filteredVouchers].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'discountValue':
        ...
        break;
      case 'expired':
        aValue = new Date(a.expired).getTime();
        bValue = new Date(b.expired).getTime();
        break;
      case 'isActive':
    ...
    <div className="min-h-screen transition-colors duration-300 bg-gradient-to-br from-green-50 to-emerald-100 dark:from-gray-900 dark:to-gray-800 p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-gray-900 dark:text-gray-100">
            <thead className="border-b border-green-100 dark:border-gray-700 bg-green-50 dark:bg-gray-800">
              <tr>
                <th
                  className="px-6 py-4 font-mono dark:text-gray-200 align-middle"
                  onClick={() => {
                    setSortField('label');
                    setSortOrder(sortField === 'label' && sortOrder === 'asc' ? 'desc' : 'asc');
                  }}
                >
                ...
                <th className="px-6 py-4 font-mono dark:text-gray-200 align-middle">
                  Đơn tối thiểu
                </th>
                <th
                  className="px-6 py-4 font-mono dark:text-gray-200 align-middle"
                  onClick={() => {
                    ...
                    setSortOrder(
                      sortField === 'discountValue' && sortOrder === 'asc' ? 'desc' : 'asc'
                    );
                  }}
                >
                  <div className="flex items-center gap-1">
                    Giá trị {getSortIcon('discountValue')}
                  </div>
                </th>
                <th
                  className="px-6 py-4 font-mono dark:text-gray-200 align-middle"
                  onClick={() => {
                    setSortField('expired');
                    setSortOrder(sortField === 'expired' && sortOrder === 'asc' ? 'desc' : 'asc');
                  }}
                >
                ...
                <th
                  ...
                  onClick={() => {
                    ...
                    setSortOrder(sortField === 'isActive' && sortOrder === 'asc' ? 'desc' : 'asc');
                  }}
                >
                  <div className="flex items-center gap-1">
              ...
              {loading ? (
                ...
                sortedVouchers.map((v, index) => {
                  ...
                  return (
                    <tr
                      key={v.id || index}
                      className="transition-colors bg-white dark:bg-gray-900 hover:bg-green-50 dark:hover:bg-gray-800 border-b border-green-100 dark:border-gray-800"
                    >
                      <td className="px-6 py-4 font-mono dark:text-gray-200 align-middle">
                        {v.label}
                      </td>
                      <td className="px-6 py-4 font-mono dark:text-gray-200 align-middle">
                        {v.code}
                      ...
                      <td
                        className="px-6 py-4 max-w-xs truncate dark:text-gray-200 align-middle"
                        title={v.description}
                      >
                        {v.description}
                      </td>
                      <td className="px-6 py-4 dark:text-gray-200 align-middle">
                        {typeof v.minOrder === 'number'
                      ...
                      <td className="px-6 py-4 dark:text-gray-200 align-middle">
                        {formatDate(v.expired)}
                      </td>
                      <td className="px-6 py-4 align-middle">
                        {isExpired ? (
                          <span className="inline-block px-2 py-1 rounded bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-xs">
                            Hết hạn
                          </span>
                        ) : v.isActive ? (
                          <span className="inline-block px-2 py-1 rounded bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 text-xs">
                            Đang hoạt động
                          </span>
                        ) : (
                          <span className="inline-block px-2 py-1 rounded bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 text-xs">
                            Tạm ngưng
                          </span>
                        )}
                      </td>
                          ...
                          >
                            ...
                            >
                              <path
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                d="M6 7.5V19.125A2.625 2.625 0 008.625 21.75h6.75A2.625 2.625 0 0018 19.125V7.5M4.5 7.5h15m-10.125 0V5.625A1.125 1.125 0 019.75 4.5h4.5a1.125 1.125 0 011.125 1.125V7.5"
                              />
                            </svg>
                          </button>
                        </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-start justify-center z-50">
          <div
            className="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-8 w-full max-w-2xl mt-12"
            style={{ position: 'relative', top: 0, transform: 'translateY(0)' }}
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    className="w-full p-3 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 border-gray-300 dark:border-gray-700"
                  />
                  <input
                    name="label"
                    value={form.label}
                  ...
                  <textarea
                    ...
                    onChange={handleInput}
                    placeholder="Mô tả"
                    className="w-full p-3 border rounded min-h-[80px] bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 border-gray-300 dark:border-gray-700"
                  />
                </div>
                <div className="space-y-4">
                  <h3 className="font-semibold text-green-600 dark:text-green-300 mb-2">
                  ...
                  <NumberInput
                    value={
                      typeof form.minOrder === 'string' ? Number(form.minOrder) || 0 : form.minOrder
                    }
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      onChange={handleInput}
                      className="w-1/2 p-3 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-700"
                    >
                    ...
                    <NumberInput
                      value={
                        typeof form.discountValue === 'string'
                  ...
                  <h3 className="font-semibold text-green-600 dark:text-green-300 mb-2 mt-4">
                    Khác
                  </h3>
                  <input
                    name="onlyOn"
                    value={form.onlyOn}
                    onChange={handleInput}
                    placeholder="Chỉ áp dụng cho (nếu có)"
                    className="w-full p-3 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 border-gray-300 dark:border-gray-700"
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-1">
                    * "Chỉ áp dụng cho" là <b>tên danh mục</b> hoặc <b>tên sản phẩm</b> (ví dụ: "Rau
                    củ" hoặc "Cà chua")
                  </p>
                  <input
                    name="note"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    value={form.expired}
                    onChange={handleInput}
                    className="w-full p-3 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-700"
                  />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-8 w-full max-w-md text-center">
            ...
            <div className="flex justify-center gap-4">
              ...
              <button
                className="px-6 py-2 bg-red-600 dark:bg-red-700 text-white rounded hover:bg-red-700 dark:hover:bg-red-800 font-semibold"
