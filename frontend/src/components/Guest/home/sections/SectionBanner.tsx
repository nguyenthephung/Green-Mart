...
export default function SectionBanner({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-3xl cursor-pointer group transition-all duration-500 hover:scale-105 hover:shadow-2xl ${className}`}
    >
      {/* Dynamic Background */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${banner.backgroundColor || sectionStyle.gradient} opacity-95`}
      />
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full">
              ...
              <span
                className={`text-sm font-bold ${sectionStyle.textColor} uppercase tracking-wider`}
              >
                {sectionType === 'sale'
                  ? 'Flash Sale'
            ...
            {/* Title */}
            <h3
              className={`text-2xl sm:text-3xl font-black ${banner.textColor || sectionStyle.textColor} mb-2 line-clamp-2 leading-tight`}
            >
              {banner.title}
            </h3>
            ...
            {banner.description && (
              <p
                className={`text-base ${banner.textColor || sectionStyle.textColor} opacity-90 line-clamp-2 leading-relaxed`}
              >
                {banner.description}
              </p>
            )}
