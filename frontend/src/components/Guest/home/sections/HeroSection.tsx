  ...
  useEffect(() => {
    const loadHeroBanners = async () => {
      try {
        ...
        const herobanners = state.banners.filter((b: any) => b.position === 'hero' && b.isActive);
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center">
          {!isLoading && (
            <div className="text-center text-white p-8">
              <h1 className="text-4xl md:text-6xl font-bold mb-4">Green Mart</h1>
              <p className="text-xl md:text-2xl mb-8">Nơi mua sắm xanh cho cuộc sống tốt đẹp hơn</p>
            </div>
          )}
        </div>
      )}
          ...
          <button
            ...
            onClick={() => {
              setCurrentHeroBanner(
                (currentHeroBanner - 1 + heroBanners.length) % heroBanners.length
              );
            }}
