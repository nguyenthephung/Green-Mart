...
const ProductCard: React.FC<ProductCardProps> = memo(
  ({
    ...
    return (
      <div
        className={`bg-white dark:bg-gray-800 p-2 sm:p-4 rounded-xl shadow-xl transform transition-all duration-400 hover:shadow-2xl dark:shadow-gray-900/50 hover:-translate-y-2 perspective-1000 relative flex flex-col h-full ${showHotBadge ? 'border-4 border-yellow-400 animate-pulse' : ''}`}
      >
        {showHotBadge && (
        ...
        >
          <Heart
            size={16}
            className={
              user && product.id && isInWishlist(product.id.toString()) ? 'fill-current' : ''
            }
          />
        </button>
        ...
        <h4 className="text-sm sm:text-lg font-medium mt-2 text-gray-900 dark:text-gray-100 line-clamp-2">
          {product.name}
        </h4>
        ...
        <div className="min-h-[38px] flex items-end">
          ...
          priceInfo.remainingStock > 0 ? (
            ...
            <div className="flex flex-wrap items-center gap-1 mt-1 w-full">
              ...
              <span className="bg-red-500 dark:bg-red-600 text-white text-xs font-semibold px-1 py-0.5 rounded whitespace-nowrap">
                SALE
              </span>
            </div>
          ) : (
