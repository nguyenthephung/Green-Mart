      ...
      {tabList.map(tab => (
        ...
        >
          <span className="flex items-center gap-2">
            ...
            {counts[tab.key] > 0 && (
              <span
                className={`inline-flex items-center justify-center min-w-[20px] h-5 px-2 rounded-full text-xs font-bold ${
                  activeTab === tab.key ? 'bg-white text-brand-green' : 'bg-brand-green text-white'
                }`}
              >
                {counts[tab.key]}
              </span>
