...
export default function OrderCard({
  ...
  return (
    <div
      className={`bg-app-card rounded-2xl border-app-border shadow-lg hover:shadow-xl transition-all duration-300 ${isMobile ? 'p-4' : 'p-6'} group`}
    >
      {/* Header with Status */}
      <div className={`flex items-center ${isMobile ? 'flex-col gap-4' : 'justify-between'} mb-6`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className={`font-semibold text-app-primary ${isMobile ? 'text-sm' : ''}`}>
              Đơn hàng #{id}
            </div>
        ...
        <div
          className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${statusInfo.bg} ${statusInfo.color}`}
        >
          <svg
            className="w-4 h-4"
            fill="none"
      ...
      {status === 'Chờ giao hàng' && (
        <div
          className={`bg-brand-green/5 rounded-xl ${isMobile ? 'p-3' : 'p-4'} mb-6 border border-brand-green/20`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className={`font-semibold text-brand-green ${isMobile ? 'text-sm' : ''}`}>
                Đang vận chuyển
              </div>
              <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-brand-green/80`}>
                {shippingStatus}
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4 mb-6">
        {items.map((item, idx) => (
          <div
            key={idx}
            className={`flex gap-4 ${isMobile ? 'p-3' : 'p-4'} bg-app-secondary rounded-xl hover:bg-app-secondary/80 transition-colors duration-200`}
          >
            <img
              src={item.image}
              alt={item.name}
            ...
            <div className="flex-1">
              <div className={`font-semibold text-app-primary mb-1 ${isMobile ? 'text-sm' : ''}`}>
                {item.name}
              </div>
              ...
              >
                ...
                <span
                  className={`text-app-secondary ${isMobile ? 'text-xs' : 'text-sm'} font-medium`}
                >
                  x{item.quantity}
                </span>
              </div>
      ...
      {/* Total */}
      <div
        className={`flex justify-between items-center mb-6 ${isMobile ? 'p-3' : 'p-4'} bg-brand-green/5 rounded-xl border border-brand-green/20`}
      >
        <span className={`text-app-primary font-medium ${isMobile ? 'text-sm' : ''}`}>
          Tổng thanh toán:
        </span>
      ...
      <div className={`flex ${isMobile ? 'flex-col gap-2' : 'justify-end gap-3'}`}>
        ...
        {cancelError && (
          <div className={`text-red-500 font-medium mt-2 ${isMobile ? 'text-sm' : ''}`}>
            {cancelError}
          </div>
        )}
      </div>
