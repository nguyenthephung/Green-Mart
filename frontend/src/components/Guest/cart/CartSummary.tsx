...
export default function CartSummary({
  ...
  return (
    <div className="bg-app-card shadow-lg rounded-2xl p-6 w-full border border-green-100">
      <h3
        className={`${isMobile ? 'text-lg' : 'text-xl'} font-semibold mb-6 text-app-primary flex items-center gap-2`}
      >
        📋 Tóm tắt đơn hàng
      </h3>
      ...
      {address && (address.fullName || address.phone) && (
        <div
          className={`mb-4 ${isMobile ? 'p-3' : 'p-4'} bg-green-50 rounded-xl border border-green-100`}
        >
          <div
            className={`${isMobile ? 'flex flex-col gap-1' : 'flex items-center gap-2'} text-sm text-green-700`}
          >
      ...
      <div className="space-y-3">
        ...
        {voucher && voucherDiscount > 0 ? (
          <div className="flex justify-between items-start gap-2 py-3 px-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
            ...
            <span className="font-semibold text-green-700 dark:text-green-300 break-all">
              -{voucherDiscount.toLocaleString()} ₫
            </span>
          </div>
        ) : (
          <div
            className="flex justify-between items-center py-3 px-4 bg-app-secondary/50 hover:bg-app-secondary/70 rounded-lg border border-app-border border-dashed transition-all duration-200 cursor-pointer group"
            onClick={onShowVoucherModal}
          >
            <span className="text-app-secondary group-hover:text-app-primary transition-colors duration-200">
              Voucher
            </span>
            <button
      ...
      <div className="border-t border-app-border mt-4 pt-4">
        <div className="flex justify-between items-center gap-2">
          ...
          <span className="text-xl font-bold text-green-600 break-all">
