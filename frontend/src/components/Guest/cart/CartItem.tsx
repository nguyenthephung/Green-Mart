...
export default function CartItem({ item, onQuantityChange, onRemove }: CartItemProps) {
  ...
  return (
    <div className={`${isMobile ? 'flex flex-col gap-3 py-3' : 'flex items-center justify-between py-4'} border-b border-app-border`}>
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h3 className={`font-medium text-app-primary break-words ${isMobile ? 'text-sm' : ''}`}>{item.name}</h3>
          
          {/* Flash Sale Badge */}
          {item.flashSale?.isFlashSale && (
            <div className="flex items-center gap-2 mb-1">
              <span className={`bg-red-500 text-white px-2 py-1 rounded ${isMobile ? 'text-xs' : 'text-xs'} font-bold`}>
          ...
          {item.flashSale?.isFlashSale ? (
            <div className="flex items-center gap-2">
              <div className={`text-red-600 font-bold ${isMobile ? 'text-sm' : ''}`}>{item.price.toLocaleString()} ₫</div>
              <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-app-muted line-through`}>{item.flashSale.originalPrice.toLocaleString()} ₫</div>
            </div>
          ) : item.originalPrice && item.originalPrice > item.price ? (
            <div className="flex items-center gap-2">
              <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-app-muted line-through`}>{item.originalPrice.toLocaleString()} ₫</div>
              <div className={`text-green-700 font-bold ${isMobile ? 'text-sm' : ''}`}>{item.price.toLocaleString()} ₫</div>
            </div>
          ) : (
            <div className={`text-green-700 font-bold ${isMobile ? 'text-sm' : ''}`}>{item.price.toLocaleString()} ₫</div>
          )}
          
          {item.unit && <div className={`${isMobile ? 'text-xs' : 'text-xs'} text-gray-500 break-words`}>Đơn vị: {item.unit}</div>}
        </div>
      </div>
      <div className={`flex items-center ${isMobile ? 'gap-4 mt-2' : 'gap-8'}`}>
        {item.type === 'weight' ? (
          <div className={`flex items-center gap-2 bg-app-input rounded-full px-2 py-1 ${isMobile ? 'ml-auto' : ''}`}>
            <input
              type="number"
              min={0.1}
              step={0.1}
          ...
          <div className={`flex items-center gap-2 bg-app-input rounded-full px-2 py-1 ${isMobile ? 'ml-auto' : ''}`}>
            ...
            <span className={`${isMobile ? 'w-8' : 'w-12'} text-center font-medium text-app-primary ${isMobile ? 'text-sm' : ''}`}>{item.quantity}</span>
            <button
