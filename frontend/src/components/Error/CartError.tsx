...
const CartError: React.FC<CartErrorProps> = ({
    ...
    switch (errorType) {
      case 'empty':
        return {
          ...
          description: message || 'Bạn chưa có sản phẩm nào trong giỏ hàng. Hãy khám phá các sản phẩm tuyệt vời của chúng tôi!',
      ...
      case 'network':
        return {
          ...
          title: 'Mất kết nối',
          description: message || 'Không thể kết nối với máy chủ. Vui lòng kiểm tra kết nối mạng và thử lại',
      ...
      case 'sync_failed':
        return {
          ...
          description: message || 'Không thể đồng bộ giỏ hàng. Một số sản phẩm có thể đã hết hàng hoặc thay đổi giá',
      ...
      default:
        return {
          ...
          description: message || 'Không thể tải thông tin giỏ hàng. Vui lòng thử lại hoặc liên hệ hỗ trợ',
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-8">
      <div className={`max-w-lg w-full bg-gradient-to-br ${config.bgGradient} rounded-3xl shadow-2xl p-8 text-center border border-white/20`}>
        
        {/* Animated Icon Container */}
        <div className="relative mb-8">
          <div className={`${config.iconBg} rounded-full w-32 h-32 flex items-center justify-center mx-auto mb-4 ${config.isEmptyState ? 'animate-pulse' : 'animate-bounce'} shadow-lg`}>
            {config.icon}
          </div>
        ...
        <div className="space-y-4">
          ...
          {config.primaryAction && (
            ...
            >
              {config.showRetry ? <RefreshCw className="w-6 h-6" /> : <ShoppingCart className="w-6 h-6" />}
