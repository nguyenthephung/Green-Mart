...
const EditProductModal: React.FC<EditProductModalProps> = ({ show, product, onClose, onSave }) => {
  ...
  const handleSave = () => {
    ...
    const updatedProduct = {
      ...editProduct,
      salePrice:
        editProduct.isSale && editProduct.discountAmount
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-4xl my-8 animate-fadeIn">
        ...
        <h2
          ...
          style={isDarkMode ? { color: '#fff' } : { color: '#15803d' }}
        >
          Sửa sản phẩm
        </h2>
            ...
            <div className="space-y-3">
              ...
              {(!Array.isArray(categories) || categories.length === 0) && (
                <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-600 mt-1">
                    Không thể tải danh mục. Bạn có thể chỉnh sửa các thông tin khác và lưu sản phẩm
                  ...
                  return (
                    <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                      ...
                      <p className="text-sm text-red-600 mt-1">
                        Danh mục "<strong>{editProduct.category}</strong>" đã bị xóa. Vui lòng chọn
                        danh mục mới cho sản phẩm này.
                      </p>
                    </div>
                  );
                }
              ...
              {(() => {
                ...
                if (
                  parentCategory &&
                  parentCat &&
                  Array.isArray(parentCat.subs) &&
                  parentCat.subs.length > 0
                ) {
                  return (
                    <div className="mt-2">
                      <label className="block text-sm font-semibold text-gray-700 mb-2">
              ...
              <NumberInput
                ...
                onChange={value =>
                  setEditProduct(prev => (prev ? { ...prev, price: value } : null))
                }
                placeholder="Giá"
              />
              {errors.price && <div className="text-red-500 text-xs">{errors.price}</div>}
              <input
                className={`w-full border px-3 py-2 rounded ${errors.stock ? 'border-red-400' : ''}`}
                placeholder="Tồn kho"
                type="number"
                value={editProduct.stock}
                onChange={e =>
                  setEditProduct(prev => (prev ? { ...prev, stock: Number(e.target.value) } : null))
                }
                style={
                  isDarkMode
                    ? { backgroundColor: '#23272f', color: '#fff', borderColor: '#374151' }
              ...
              <select
                ...
                onChange={e =>
                  setEditProduct(prev => (prev ? { ...prev, unit: e.target.value } : null))
                }
              ...
              <input
                ...
                onChange={e =>
                  setEditProduct(prev => (prev ? { ...prev, brand: e.target.value } : null))
                }
                style={
                  isDarkMode
                    ? { backgroundColor: '#23272f', color: '#fff', borderColor: '#374151' }
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    style={
                      isDarkMode
                        ? { backgroundColor: '#f3f4f6', color: '#23272f', borderColor: '#e5e7eb' }
                        : {}
                    }
              ...
              <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <NumberInput
                          ...
                          onChange={value =>
                            setEditProduct(prev =>
                              prev ? { ...prev, discountAmount: value } : null
                            )
                          }
                        ...
                        <div className="bg-green-50 border border-green-200 rounded p-2">
                          <div className="text-xs text-gray-600">
                            ...
                            <span className="font-medium">
                              {editProduct.price?.toLocaleString()}đ
                            </span>
                          </div>
                          <div className="text-xs text-red-600">
                            Giảm:{' '}
                            <span className="font-medium">
                              {editProduct.discountAmount.toLocaleString()}đ
                            </span>
                          </div>
                          <div className="text-sm font-bold text-green-700">
                            Giá bán:{' '}
                            {(
                              (editProduct.price || 0) - editProduct.discountAmount
                            ).toLocaleString()}
                            đ
                          </div>
                        </div>
                ...
                <div className="flex-1 bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex flex-col justify-center">
                  ...
                  <div className="text-xs text-gray-500">
                    Đánh dấu để sản phẩm xuất hiện ở mục nổi bật trên trang chủ.
                  </div>
                </div>
              </div>
        ...
        <button
          className="w-full bg-green-600 text-white py-2 rounded font-semibold hover:bg-green-700 mt-4 flex items-center justify-center gap-2"
          onClick={handleSave}
        >
