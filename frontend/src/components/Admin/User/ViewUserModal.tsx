        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-gray-900 dark:text-white">
                {user.status === 'active' ? '✅ Hoạt động' : user.status === 'suspended' ? '🚫 Tạm khóa' : '⏸️ Không hoạt động'}
              </p>
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Cập nhật cuối</label>
              <p className="text-gray-900 dark:text-white">{new Date(user.updatedAt).toLocaleString('vi-VN')}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-semibold text-green-600 dark:text-green-400">{formatPrice(user.totalSpent)}</p>
            </div>
            {user.lastLogin && (
          ...
          {user.vouchers && Object.keys(user.vouchers).length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Voucher sở hữu</label>
