...
const normalizeStatus = (status: string) => {
  ...
  if (s.includes('giao') || s.includes('shipping') || s.includes('vận chuyển')) return 'Chờ giao hàng';
  if (s.includes('xác nhận') || s.includes('chờ') || s.includes('processing') || s.includes('pending')) return 'Chờ xác nhận';
  return 'Chờ xác nhận';
};
